...
if action is None:
    ...
    navigator.navigator().getAllCategories()

elif action == 'extract_series':
    navigator.navigator().extractSeries(url, img_url, hun_title, content, ep_title, year)
    
elif action == 'extract_movie':
    navigator.navigator().extractMovie(url, img_url, hun_title, content, year)    

elif action == 'extract_episodes':
    navigator.navigator().extractEpisodes(url, img_url, hun_title, content, ep_title, year)
    ...
    navigator.navigator().getMovieItems(url, img_url, hun_title, content, year)

elif action == 'series_items':
    navigator.navigator().getSeriesItems(url, img_url, hun_title, year)
